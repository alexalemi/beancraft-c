...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --decode-tally      Read a tally.bc register bank (R=VALUE) back into named registers\n"
...
static struct option long_options[] = {
    ...
    {"decode-tally", no_argument,     NULL, 'y'},
...
int main(int argc, char *argv[]) {
    ...
    if (emit_tally || decode_tally) {
        // The encoding examples/tally.bc reads (src/tally.c): the program as
        // a tally string P, the registers as one number R = prod p_r^v_r.
        TallyLayout lay;
        tally_layout(arena, prog, &lay);

        // REG=VALUE arguments (bignums) -- the initial bank for --emit-tally,
        // or the bank to decode for --decode-tally (its R=... argument).
        Bignum *vals = arena_alloc(arena, (prog->reg_count ? prog->reg_count : 1) * sizeof(Bignum));
        for (uint32_t i = 0; i < prog->reg_count; i++) vals[i] = bignum_from_u64(prog->reg_init[i]);
        Bignum bank = bignum_zero();
        bool have_bank = false;
        for (int i = optind + 1; i < argc; i++) {
            char *eq = strchr(argv[i], '=');
            if (!eq) continue;
            *eq = '\0';
            if (decode_tally && strcmp(argv[i], "R") == 0) {
                bignum_free(&bank);
                bank = bignum_from_string(eq + 1);
                have_bank = true;
            } else {
                int32_t idx = ir_find_reg(prog, argv[i]);
                if (idx >= 0) { bignum_free(&vals[idx]); vals[idx] = bignum_from_string(eq + 1); }
                else fprintf(stderr, "Warning: unknown register '%s'\n", argv[i]);
            }
            *eq = '=';
        }
        ...
        if (decode_tally) {
            if (!have_bank) {
                fprintf(stderr, "Error: --decode-tally needs the bank as R=VALUE\n");
                arena_free(arena);
                return 1;
            }
            Bignum *out = arena_alloc(arena, (prog->reg_count ? prog->reg_count : 1) * sizeof(Bignum));
            Bignum rest = tally_decode(&lay, bank, out);
            printf("Results:\n");
            for (uint32_t i = 0; i < lay.used_count; i++) {
                uint32_t r = lay.by_prime[i];
                if (prog->reg_names[r]->data[0] == ':') continue;
                char *v = bignum_to_string(out[r]);
                printf("%s = %s\n", prog->reg_names[r]->data, v);
                free(v);
            }
            if (!bignum_eq(rest, bignum_from_u64(1))) {
                char *v = bignum_to_string(rest);
                fprintf(stderr, "Warning: R has a leftover factor %s that no register's prime divides\n", v);
                free(v);
            }
            arena_free(arena);
            return 0;
        ...
        Bignum P = tally_program(&lay);
        Bignum R = tally_bank(&lay, vals);
        printf("# %s -> tally.bc encoding: %u instructions, %zu bits.  registers (prime):",
               filename, lay.inst_count, lay.bit_count);
        for (uint32_t i = 0; i < lay.used_count; i++) {
            uint32_t r = lay.by_prime[i];
            printf(" %s=%" PRIu64, prog->reg_names[r]->data, lay.prime_of[r]);
        }
        printf("\n");
